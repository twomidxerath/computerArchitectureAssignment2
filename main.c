...
static void print_best_results(
    ...
    for (cl = 0; cl < NUM_CACHE; cl++) {
        ...
        double best_i_missrate = 0.0, best_i_total_cycles = 0.0;

        const char *best_d_policy = "N/A";
        int best_d_block = 0, best_d_assoc = 0;
        double best_d_missrate = 0.0, best_d_total_cycles = 0.0;
        int best_d_writes = 0;
        ...
        for (bl = 0; bl < NUM_BLOCK; bl++) {
            for (as = 0; as < NUM_ASSOC; as++) {
                ...
                double lru_i_cycles = (lru_i_total * (1.0 - lru_i_mr) * i_hit) + (lru_i_total * lru_i_mr * i_miss);

                if (lru_i_cycles < min_i_cycles) {
                    min_i_cycles = lru_i_cycles;
                    best_i_policy = "LRU"; best_i_block = BLOCK_SIZES[bl]; best_i_assoc = ASSOC_LIST[as];
                    best_i_missrate = lru_i_mr; best_i_total_cycles = lru_i_cycles;
                }

                double lru_d_mr = lru_miss[row_d][col];
                int lru_d_total = lru_d_tot[row_d][col];
                double lru_d_cycles = (lru_d_total * (1.0 - lru_d_mr) * d_hit) + (lru_d_total * lru_d_mr * d_miss);

                if (lru_d_cycles < min_d_cycles) {
                    min_d_cycles = lru_d_cycles;
                    best_d_policy = "LRU"; best_d_block = BLOCK_SIZES[bl]; best_d_assoc = ASSOC_LIST[as];
                    best_d_missrate = lru_d_mr; best_d_writes = lru_writes[row_d][col];
                ...
                double fifo_i_cycles = (fifo_i_total * (1.0 - fifo_i_mr) * i_hit) + (fifo_i_total * fifo_i_mr * i_miss);

                if (fifo_i_cycles < min_i_cycles) {
                    min_i_cycles = fifo_i_cycles;
                    best_i_policy = "FIFO"; best_i_block = BLOCK_SIZES[bl]; best_i_assoc = ASSOC_LIST[as];
                    best_i_missrate = fifo_i_mr; best_i_total_cycles = fifo_i_cycles;
                }

                double fifo_d_mr = fifo_miss[row_d][col];
                int fifo_d_total = fifo_d_tot[row_d][col];
                double fifo_d_cycles = (fifo_d_total * (1.0 - fifo_d_mr) * d_hit) + (fifo_d_total * fifo_d_mr * d_miss);

                if (fifo_d_cycles < min_d_cycles) {
                    min_d_cycles = fifo_d_cycles;
                    best_d_policy = "FIFO"; best_d_block = BLOCK_SIZES[bl]; best_d_assoc = ASSOC_LIST[as];
                    best_d_missrate = fifo_d_mr; best_d_writes = fifo_writes[row_d][col];
